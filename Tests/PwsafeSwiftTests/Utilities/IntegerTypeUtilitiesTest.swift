    ...
    override func spec() {
        describe("UInt32") {
            it("should load from empty byte array") {
                let bytes: [UInt8] = [];
                expect(UInt32(littleEndianBytes: bytes)) == 0
            }
            
            it("should load from 4-byte array") {
                let bytes: [UInt8] = [1, 2, 3, 4]
                expect(UInt32(littleEndianBytes: bytes)) == 0x04030201
            }

            it("should load from small byte array") {
                let bytes: [UInt8] = [1, 2, 3]
                expect(UInt32(littleEndianBytes: bytes)) == 0x030201
            }
        ...
        describe("UInt16") {
            it("should load from empty byte array") {
                let bytes: [UInt8] = []
                expect(UInt16(littleEndianBytes: bytes)) == 0
            }
            
            it("should load from 4-byte array") {
                let bytes: [UInt8] = [1, 2]
                expect(UInt16(littleEndianBytes: bytes)) == 0x0201
            }
            
            it("should load from small byte array") {
                let bytes: [UInt8] = [1]
                expect(UInt16(littleEndianBytes: bytes)) == 0x01
            }
            ...
            it("should return little endian byte array") {
                ...
                expect(value.littleEndianBytes()) == [0x02, 0x01]
            }
        }
    }
}
