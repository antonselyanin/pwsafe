    ...
    override func spec() {
        ...
        describe("reads bytes from data") {
            ...
            it("reads 3 bytes") {
                ...
                // Then
                expect(parsed.remainder) == Data([3, 4, 5, 6])
                expect(parsed.value) == Data([0, 1, 2])
            }
        ...
        describe("expect string") {
            let data: Data = Data("PWS".utf8Bytes() + [3, 4, 5, 6])
        ...
        describe("read all") {
            ...
            it("reads and leave last N bytes") {
                ...
                expect(parsed.value) == Data([0, 1, 2, 3, 4])
            }
        ...
        describe("read ByteArrayConvertible") {
            let data: Data = Data([0x04, 0x03, 0x02, 0x01, 0xFF])
            ...
            it("reads UInt32") {
                ...
                // Then
                expect(parsed.remainder) == Data([0xFF])
        ...
        describe("Parser<Data>.bytes") {
            it("converts to bytes") {
                // Given
                let parser = Parser<Data>.pure(Data([0x01, 0x02, 0x03]))
        ...
        describe("ParserProtocol.many") {
            it("returns array") {
                ...
                let parsed = parser.parse(Data([0, 1, 2, 3, 4])).value!
                ...
                expect(parsed.remainder) == Data([4])
            }
            
            it("return empty if can't parse") {
                // When
                let parser = twoBytesReader.many
                let parsed = parser.parse(Data([1])).value!
                
                // Then
                expect(parsed.value.count) == 0
                expect(parsed.remainder) == Data([1])
            }
        }
        ...
        describe("ParserProtocol.aligned(blockSize:)") {
            ...
            it("ensures that data is read in blocks of specified size") {
                ...
                // Then
                expect(parsed.remainder) == Data([3, 4])
            ...
            it("doesn't align if reading already aligned") {
                ...
                // Then
                expect(parsed.remainder) == Data([2, 3, 4])
                expect(parsed.value) == [0, 1]
            }
